...
func TestIntegration_ExternalInitiator(t *testing.T) {
	// TODO(felder-cl): authenticate posts to external initiator
	// TODO(felder-cl): when e.i. name not found, return informative error
	// TODO(felder-cl): allow one-to-many external initiators
	// TODO(felder-cl): handle case where Remote external initiator received post, but database failed.
	t.Parallel()
